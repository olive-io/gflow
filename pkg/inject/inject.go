...
func InitGraph(logger Logger) {
	g = NewGraph(logger)
}

func Provide(vv ...any) error {
	for _, v := range vv {
...
func ProvidePanic(vv ...any) {
	if err := Provide(vv...); err != nil {
		panic(err)
	}
}

func ProvideWithName(v any, name string) error {
	return g.Provide(&Object{Value: v, Name: name})
}

func ProvideWithNamePanic(v any, name string) {
	if err := ProvideWithName(v, name); err != nil {
		panic(err)
	}
}

func PopulateTarget(target any) error {
...
func Resolve(dst any) error {
	return g.Resolve(dst)
}

func ResolveByName(dst any, name string) error {
...
func NewGraph(logger Logger) *Graph {
	return &Graph{
		Logger:      logger,
		unnamed:     make([]*Object, 0),
		unnamedType: make(map[reflect.Type]bool),
...
func (g *Graph) PopulateTarget(dst interface{}) error {
...
func (g *Graph) populateExplicit(o *Object, provide bool) error {
...
StructLoop:
	for i := 0; i < o.reflectValue.Elem().NumField(); i++ {
		...
		if provide {
			...
			err = g.PopulateTarget(newObject.Value)
			if err != nil {
				return err
			}
		}
